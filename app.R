...
server <- function(input, output, session){
  ...
  super_PC <- reactive({
    if(!is.null(data_input2()))
    {
      ...
      withProgress(message = 'Iterations', value = 0, {
        ...
        sup <- superpc(data1, data2, nstep = as.integer(input$cutoff1), prop = as.numeric(as.integer(input$split1)/100), nfold = as.integer(input$nfold3), cv.order = cv.order, splitIndex = splitIndex )
      })
  ...
  output$SuperPC <- DT::renderDataTable({
        DT::datatable(super_PC()[[3]], options = list(
  ...
  output$predictplot <- renderPlot({
    ...
    if (input$Predicted1 == 1) {
      ...
      barplot(counts, col=c("blue","grey"), main = "Barplot between Observed and Predicted", 
              legend = rownames(counts), beside=TRUE, args.legend = list(x = "topleft"))
      
    } else if (input$Predicted1 == 0) {
      boxplot(dat$pred.contiuous~dat$group_label, main = "Box plot of Predicted Principal Component between Observed groups")
    } 
    })
  ...
  output$ex_out1 <- renderPrint({
    ...
    if (input$Predicted1 == 1) {
      table(dat$pred.discrete, dat$group_label)
      
    } else if (input$Predicted1 == 0) {
      tapply(dat$pred.contiuous, dat$group_label, summary)
    } 
  })
  ...
  output$ex_out2 <- renderPrint({
    ...
    if (input$Predicted1 == 1) {
      ...
      t.test(dat$pred.contiuous~dat$group)
    } 
  })
  
  output$ex_p <- renderPrint({
    cat(paste("The p-value for the most significant Principal component is = ", super_PC()[[1]], ".", sep = " "))
  })
    ...
    if(!is.null(data_input4()) | !is.null(data_input5()))
    {
      ...
      if (input$foldid2 == 0) {
        ...
        withProgress(message = 'Iterations', value = 0, {
          ...
          sup <- superpccox(data3, data4, nstep = as.integer(input$cutoff2), prop = as.numeric(as.integer(input$split2)/100), nfold = as.integer(input$nfold1), cv.order = cv.order, sub=sub )
  ...
  output$SuperPC2 <- DT::renderDataTable({
    DT::datatable(super_PC2()[[3]], options = list(
      lengthMenu = list(c(5, 10, -1), c('5', '10', 'All')),
      pageLength = 10))
  })
  ...
  output$ex_p2 <- renderPrint({
    cat(paste("The p-value for the first Principal component is = ", super_PC2()[[1]], ".", sep = ""))
  })
  ...
  super_PC3 <- reactive({
    if(!is.null(data_input6()) | !is.null(data_input7()))
    {
      ...
      if (input$foldid3 == 0) {
        ...
        sub = NULL
        withProgress(message = 'Iterations', value = 0, {
          sup <- superpccon(data5, data6, nstep = as.integer(input$cutoff3), prop = as.numeric(as.integer(input$split3)/100), nfold = as.integer(input$nfold2), cv.order = cv.order, sub=sub)
  ...
  output$SuperPC3 <- DT::renderDataTable({
    DT::datatable(super_PC3()[[3]], options = list(
      lengthMenu = list(c(5, 10, -1), c('5', '10', 'All')),
      pageLength = 10))
  })
  ...
  output$predictplot3 <- renderPlot({
    ...
    if (input$Predicted2 == 1) {
      boxplot(dat$outcome~dat$pred.discrete, main = "Box plot of Predicted Principal Component between Observed Outcome")
      
    } else if (input$Predicted2 == 0) {
      outcome = dat$outcome
      predicted = dat$pred.contiuous
      plot(outcome, predicted, main = "Scatter plot of Predicted Principal Component and Observed Outcome")
    } 
  })
  ...
  output$ex_out3 <- renderPrint({
    ...
    if (input$Predicted2 == 1) {
      t.test(dat$outcome~dat$pred.discrete)
      
    } else if (input$Predicted2 == 0) {
      outcome = dat$outcome
      predicted = dat$pred.contiuous
      cor.test(outcome, predicted)
    } 
  })
 
  output$ex_p3 <- renderPrint({
    cat(paste("The p-value for the first Principal component is = ", super_PC3()[[1]], ".", sep = ""))
  })
