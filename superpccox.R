superpccox <- function(file1, file2, nstep, prop, nfold, cv.order, sub){
  ...
  train.data<-list(x=train.x.t,y=train.y,censoring.status=train.censor,featurenames=train.featurenames)
  ...
  test.data<-list(x=test.x.t,y=test.y,censoring.status=test.censor,featurenames=test.featurenames)
  
  # train  the model for survival data.
  # This step just computes the  scores for each feature.
  train.obj<-superpc.train(train.data, type="survival")
  
  pcm_res <- pcm.cv2(train.data, train.obj$feature.scores, n.fold = nfold, n.step = nstep, cv.order= cv.order)
  pcm <- pcm_res[[1]]
  ...
  pvalue <- res$coeftable[1, 5]
  
  km <- survfit(Surv(test.data$y,test.data$censoring.status)~fit.groups$v.pred)
  ...
  results <- list(pvalue, km, res_data, foldid)
